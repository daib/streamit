 ...
 *
 * $Id: LLkAnalyzer.java,v 1.1 2009/02/24 18:15:01 hormati Exp $
 */
 ...
 * @see     at.dms.compiler.tools.antlr.compiler.Grammar
 * @see     at.dms.compiler.tools.antlr.compiler.Lookahead
 */
public class LLkAnalyzer implements LLkGrammarAnalyzer {
    ...
    private CharFormatter charFormatter = new JavaCharFormatter();

    /**
     * Create an LLk analyzer
     */
    public LLkAnalyzer(Main tool_) {
        tool = tool_;
    }
    ...
    public boolean deterministic(AlternativeBlock blk) {
        ...
        if ( blk.greedy==false && !(blk instanceof OneOrMoreBlock) && !(blk instanceof ZeroOrMoreBlock) ) {
            Utils.warning("Being nongreedy only makes sense for (...)+ and (...)*", grammar.getFilename(), blk.getLine());
        }
    ...
    public boolean deterministicImpliedPath(BlockWithImpliedExitPath blk) {
        ...
        for (int i=0; i<nalts; i++) {       // check follow against all alts
            ...
            if ( alt.head instanceof BlockEndElement ) {
                Utils.warning("empty alternative makes no sense in (...)* or (...)+", grammar.getFilename(), blk.getLine());
            }
    ...
    public Lookahead FOLLOW(int k, RuleEndElement end) {
        ...
        String rule;
        if (lexicalAnalysis) {
            rule = JavaCodeGenerator.lexerRuleName(rb.getRuleName());
        } else {
            rule = rb.getRuleName();
        }
    ...
    public Lookahead look(int k, BlockEndElement end) {
         ...
         * lookahead."
         */
        if ( end.block instanceof SynPredBlock ) {
            p.setEpsilon();
        }
    ...
    public Lookahead look(int k, CharLiteralElement atom) {
        ...
        if (lexicalAnalysis) {
            ...
            Utils.panic("Character literal reference found in parser");
                          ...
                          String linePrefix)
    {
        for (int i = 1; i <= depth; i++) {
            ...
            if (lexicalAnalysis) {
                String bits = sets[i].fset.toStringWithRanges(",", charFormatter);
     ...
     */
    private void warnAltAmbiguity(Grammar grammar,
                                  ...
                                  int altIdx2)
    {
        ...
        if ( blk instanceof RuleBlock && ((RuleBlock)blk).isLexerAutoGenRule() ) {
            ...
            String ri = JavaCodeGenerator.reverseLexerRuleName(rri.targetRule);
            String rj = JavaCodeGenerator.reverseLexerRuleName(rrj.targetRule);
     ...
     */
    private void warnAltExitAmbiguity(Grammar grammar,
