 ...
 *
 * $Id: MakeGrammar.java,v 1.1 2009/02/24 18:15:01 hormati Exp $
 */
...
public class MakeGrammar extends DefineGrammarSymbols {
    ...
    public MakeGrammar(Main tool, LLkAnalyzer analyzer) {
        super(tool, analyzer);
    }
    /**
        ...
        for (int i=0; i<lexRules.size(); i++) {
            ...
            if (!r.isDefined()) {
                ...
                if ( r.access.equals("public") ) {
                    ...
                    RuleRefElement rr =
                        new RuleRefElement(g, new CommonToken(ANTLRTokenTypes.RULE_REF, r.getId()));
    ...
    private AlternativeBlock createOptionalRuleRef(String rule, int line) {
        ...
        String mrule = JavaCodeGenerator.lexerRuleName(rule); // can only be a lexer rule!
        ...
        t.setLine(line);
        RuleRefElement rref = new RuleRefElement(grammar, t);
        ...
        throws SemanticException
    {
        ...
        super.defineRuleName(r, access, docComment);
        String id = r.getText();
        ...
        RuleBlock rb = new RuleBlock(grammar, r.getText(), r.getLine());
    ...
    public void refCharLiteral(Token lit, Token label, boolean inverted, boolean lastInRule) {
        ...
        super.refCharLiteral(lit, label, inverted, lastInRule);
        CharLiteralElement cl = new CharLiteralElement((LexerGrammar)grammar, lit, inverted);
        ...
        if (
            ...
            Utils.warning("Character literal must be lowercase when caseSensitive=false", grammar.getFilename(), lit.getLine());
        }
    ...
    public void refCharRange(Token t1, Token t2, Token label, boolean lastInRule) {
        ...
        if (!((LexerGrammar)grammar).caseSensitive) {
            if (rangeMin < 128 && Character.toLowerCase((char)rangeMin) != (char)rangeMin) {
                Utils.warning("Character literal must be lowercase when caseSensitive=false", grammar.getFilename(), t1.getLine());
            }
            if (rangeMax < 128 && Character.toLowerCase((char)rangeMax) != (char)rangeMax) {
                Utils.warning("Character literal must be lowercase when caseSensitive=false", grammar.getFilename(), t2.getLine());
            }
        }

        super.refCharRange(t1, t2, label, lastInRule);
        CharRangeElement cr = new CharRangeElement((LexerGrammar)grammar, t1, t2);
    ...
    public void refReturnAction(Token returnAction) {
        if (grammar instanceof LexerGrammar) {
            ...
            if (rs.access.equals("public")) {
                Utils.warning("public Lexical rules cannot specify return type", grammar.getFilename(), returnAction.getLine());
                return;
            }
        }
    ...
    public void refRule(Token idAssign, Token r, Token label, Token args) {
        ...
        super.refRule(idAssign, r, label, args);
        lastRuleRef = new RuleRefElement(grammar, r);
        if (args != null) {
        ...
        if (grammar instanceof LexerGrammar && !((LexerGrammar)grammar).caseSensitive) {
            for (int i = 1; i < lit.getText().length()-1; i++) {
                ...
                if (c < 128 && Character.toLowerCase(c) != c) {
                    Utils.warning("Characters of string literal must be lowercase when caseSensitive=false", grammar.getFilename(), lit.getLine());
                    break;
                }
            }
        }
    ...
    public void refToken(Token idAssign, Token t, Token label, Token args,
        ...
        if (grammar instanceof LexerGrammar) {
            ...
            super.refToken(idAssign, t, label, args, inverted, lastInRule);
            TokenRefElement te = new TokenRefElement(grammar, t, inverted);
    ...
    public void refTokenRange(Token t1, Token t2, Token label, boolean lastInRule) {
        ...
        super.refTokenRange(t1, t2, label, lastInRule);
        TokenRangeElement tr = new TokenRangeElement(grammar, t1, t2);
        if (tr.end < tr.begin) {
            tool.error("Malformed range.", grammar.getFilename(), t1.getLine());
            return;
        }
