...
public class E2StaticStreamGraph extends ScheduledStaticStreamGraph {
    /**
     * create a static stream graph with realTop as the first node.
     */
     public E2StaticStreamGraph(StreamGraph sg, FlatNode realTop) {
         super(sg,realTop);
     }
     ...
     public void cleanupForFused() {
         ...
         for (FlatNode node : flatNodes) {
             if (E2Fusion.isEliminated(node)) {
                 node = E2Fusion.getMaster(node);
             }
         ...
         for (int i=0; i < outputs.length; i++) {
              if (! E2Fusion.isEliminated(outputs[i])) {
                  newOutputs.add(outputs[i]);
              } else {
                  newOutputs.add(E2Fusion.getMaster(outputs[i]));
              }
         }
         ...
         for (int i=0; i < inputs.length; i++) {
             if (! E2Fusion.isEliminated(inputs[i])) {
                 newInputs.add(inputs[i]);
             }
             else {newInputs.add(E2Fusion.getMaster(inputs[i]));}
        }
         inputs = newInputs.toArray(new FlatNode[]{});
         
         if (E2Fusion.isEliminated(topLevel)) {
             topLevel = E2Fusion.getMaster(topLevel);
         }
