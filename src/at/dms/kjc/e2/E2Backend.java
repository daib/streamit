...
//    static streamit.scheduler2.iriter.Iterator topStreamIter; 
     ...
     */
    static E2StreamGraph streamGraph;
...
//        }
        ...
        streamGraph = new E2StreamGraph((new GraphFlattener(str)).top);
        //create the static stream graphs cutting at dynamic rate boundaries
        streamGraph.createStaticStreamGraphs();
        //E2StaticStreamGraph[] ssgs = (E2StaticStreamGraph [])(streamGraph.getStaticSubGraphs());
        ...
        for (int k = 0; k < numSsgs; k++) {
            E2StaticStreamGraph ssg = (E2StaticStreamGraph)streamGraph.getStaticSubGraphs()[k];
            if (numSsgs>1) {
...
//            }
                        ...
                        while (opIter.hasNext()) {
                            if (opIter.next() == self && opIter.hasNext()) {
                                ...
                                if (op instanceof SIRFilter) {
                                    ...
                                    if (Arrays.equals(E2Backend.originalRates.get(filter), new Integer[]{1,2,2}) ||
                                        Arrays.equals(E2Backend.originalRates.get(filter), new Integer[]{4,8,8})) {
            ...
            E2Fusion.setPartitionMap(partitionMap);

            if (KjcOptions.fusion) {
                FlatNode TopBeforeFusion = ssg.getTopLevel();
                TopBeforeFusion.accept(new E2Fusion(), new HashSet<FlatNode>(), true);
        ...
        if (numSsgs==1) {
            E2StaticStreamGraph ssg = (E2StaticStreamGraph)streamGraph.getStaticSubGraphs()[0];
