...
public class TapeCluster extends TapeBase implements Tape {
	...
	/**
	 * Consumer name for init code in ClusterCodeGeneration. Do not use
	 * elsewhere.
	 */
	 ...
	 * 
	 * @see
	 * at.dms.kjc.cluster.Tape#dataDeclarationH(at.dms.kjc.common.CodegenPrintWriter
	 * )
	 */
	@Override
	 ...
	 * 
	 * @see
	 * at.dms.kjc.cluster.Tape#dataDeclaration(at.dms.kjc.common.CodegenPrintWriter
	 * )
	 */
	@Override
	 ...
	 * 
	 * @see
	 * at.dms.kjc.cluster.Tape#downstreamDeclarationExtern(at.dms.kjc.common
	 * .CodegenPrintWriter)
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see at.dms.kjc.cluster.Tape#downstreamDeclaration(at.dms.kjc.common.
	 * CodegenPrintWriter)
	 */
	@Override
	public String downstreamDeclaration() {
		...
		if (my_node.contents instanceof SIRFilter) {
			...
			/*
			 * __pop_buf_DST, __head__DST, __tail__DST, __init_pop_buf__DST seem
			 * to have to do with copying out any data items that are in the
			 * input buffer that are not popped in the current iteration
			 */
			...
			s.append("inline void __init_pop_buf__");
			s.append(dst);
			s.append("() {\n");
			...
			s.append("void save_peek_buffer__" + dst
			...
			if (peek_n <= pop_n) {
				...
				if (pop_n > 0) {
					...
					// s.append(popExpr()+";\n");
					FlatNode src_node = NodeEnumerator.getFlatNode(src);
					if (E2Fusion.fusedWith(my_node).contains(src_node)) {
						s.append("__producer_" + getSource() + "_" + getDest()
								+ ".front();\n");
						s.append("\t__producer_" + getSource() + "_"
								+ getDest() + ".pop();\n");
					} else
						s.append(popExpr() + ";\n");
					if (pop_n > 1) {
			...
			// p.indent();
			if (peek_n <= pop_n) {
				s.append("return __pop_buf__" + dst + "[__tail__" + dst
						+ "++];\n");
			} else {
				s.append(typeString + " res=__pop_buf__" + dst + "[__tail__"
						+ dst + "];");
			...
			// List<Tape> out =
			// RegisterStreams.getNodeOutStreams(my_node.contents);
			//
			...
			// // if push_buffer is full then copies correct portion to each
			// output tapes.
			// s.append(CommonUtils.declToString(type, push_buffer + "[" +
			// sum_of_weights + "]", true) + ";\n");
			// s.append("int " + push_index + " = 0;\n");
			// s.append("\n");
			// s.append("void " + push_name +"("+ typeString + " data" +
			// ") {\n");
			// s.append("  " + push_buffer + "[" + push_index +
			// "++] = data;\n");
		...
		if (my_node.contents instanceof SIRSplitter
				|| my_node.contents instanceof SIRJoiner) {
			// Janis had simple pop for splitters / joiners coded inline.
			s.append("inline " + typeString + " " + pop_name + "() {\n");

			// if node is fused, when use different buffer scheme
			if (E2Fusion.isEliminated(my_node)
					&& my_node.contents instanceof SIRSplitter) {
				String inputQueueName = "__push_" + getSource() + "_"
						+ getDest() + "_queue";
				s.append("extern std::queue<"
						+ CommonUtils.CTypeToStringA(getType(), true) + "> "
						+ inputQueueName + ";\n");
				s.append("   " + CommonUtils.CTypeToStringA(getType(), true)
						+ " tmp = " + inputQueueName + ".front();\n");
				s.append("   " + inputQueueName + ".pop();\n");
				s.append("   return tmp;\n");
			} else
	...
	public String pushManyItems(String sourceBuffer, int sourceOffset,
			int numItems) {
		StringBuffer s = new StringBuffer();
		s.append(producer_name + ".push_items(&" + sourceBuffer + "["
	...
	public String popManyItems(String destBuffer, int destOffset, int numItems) {
		StringBuffer s = new StringBuffer();
		s.append("    " + consumer_name + ".pop_items(&" + destBuffer + "["
				+ destOffset + "], " + numItems + ");\n");
		return s.toString();
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see at.dms.kjc.cluster.Tape#upstreamDeclarationExtern(at.dms.kjc.common.
	 * CodegenPrintWriter)
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see at.dms.kjc.cluster.Tape#upstreamDeclaration(at.dms.kjc.common.
	 * CodegenPrintWriter)
	 */
	@Override
	public String upstreamDeclaration() {
		...
		if (node.isFilter()) {
			...
			if (push_n > 0) {
				...
				out_pop_buffer_size = Math.max(2, 40960 / f.getOutputType()
						.getSizeInC());
			} else {
			...
			s.append(typeString + " " + pop_buffer + "[" + out_pop_buffer_size
					+ "];\n" + "int " + pop_index + " = "
			...
			if (!(E2Fusion.isEliminated(dstNode)& dstNode.contents  instanceof SIRSplitter)) {
				s.append("\twhile(" + push_name + "_queue.size() > 0) {\n");
				s.append("\t\t" + pushPrefix() + push_name + "_queue.front()"
						+ pushSuffix() + ";\n");
				s.append("\t\t" + push_name + "_queue.pop();\n");
				s.append("\t}\n");
			}

			s.append("}\n"); // end update push buffer
			 ...
			 * createPushRoutineBody(s, "data"); //p.outdent(); s.append("}\n");
			 */

			// (1) WTF. (2) can not use pop_name if pop routine at downstream
			// end uses pop_name
			...
			// // this is presumably "pop" for a node downstream of the
			// splitter.
			// // This routine is a bit disgusting: if pop is called for a
			// particular
			// // downstream node, then all the splitters other downstream nodes
			// have
			...
			// List<Tape> splitterOuts =
			// RegisterStreams.getNodeInStreams(splitter);
			//
			//
			// if (splitter.getType().equals(SIRSplitType.DUPLICATE)) {
			// s.append(typeString + " " + pop_name + "() {\n");
			...
			// if (splitterOut != this) {
			// s.append("    "+ splitterOut.pushPrefix() + "tmp" +
			// splitterOut.pushSuffix() + ";\n");
			// }
			// }
			// s.append("    return tmp;\n");
			//
			...
			// This code seems redundant with code created in ClusterCode for
			// joiner work
	 ...
	 * (non-Javadoc)
	 * 
	 * @see at.dms.kjc.cluster.Tape#topOfWorkIteration(at.dms.kjc.common.
	 * CodegenPrintWriter)
	 */
	...
	public String downstreamCleanup() {
		return consumer_name + ".delete_socket_obj();\n";
	}
	 ...
	 * 
	 * @see
	 * at.dms.kjc.cluster.Tape#pushPrefix(at.dms.kjc.common.CodegenPrintWriter)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * at.dms.kjc.cluster.Tape#pushSuffix(at.dms.kjc.common.CodegenPrintWriter)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * at.dms.kjc.cluster.Tape#popPrefix(at.dms.kjc.common.CodegenPrintWriter)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * at.dms.kjc.cluster.Tape#popNStmt(at.dms.kjc.common.CodegenPrintWriter)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * at.dms.kjc.cluster.Tape#peekPrefix(at.dms.kjc.common.CodegenPrintWriter)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * at.dms.kjc.cluster.Tape#peekSuffix(at.dms.kjc.common.CodegenPrintWriter)
	 */
	@Override
	...
	/*
	 * No cleanup statements (non-Javadoc)
	 * 
	...
	public String assignPopToVar(String varName) {
		if (type instanceof CArrayType) {
			return "memcpy(" + varName + ", " + pop_name + "(), " + "sizeof("
					+ varName + "));\n";
		} else {
	...
	public String assignPeekToVar(String varName, String offset) {
		if (type instanceof CArrayType) {
			return "memcpy(" + varName + ", " + peek_name + "(" + offset
					+ "), " + "sizeof(" + varName + "));\n";
		} else {
