...
public class E2Code {
        ...
        public void visitNode(FlatNode node) 
        {
            ...
            if (node.contents instanceof SIRFilter) {
                ...
                if (KjcOptions.compressed && 
                        (Arrays.equals(E2Backend.originalRates.get(filter), new Integer[]{1,1,1}) ||
                         Arrays.equals(E2Backend.originalRates.get(filter), new Integer[]{4,4,4})) &&
                    ...
                    if (E2Backend.originalRates.get(filter)[0] == 1) {
                    ...
                    filter.addMethod(oldWork);
                            ...
                            !at.dms.kjc.sir.lowering.fission.StatelessDuplicate.hasMutableState(filter)) {
                    int push = E2Backend.originalRates.get(filter)[0];
                    int pop = E2Backend.originalRates.get(filter)[1];
                    int peek = E2Backend.originalRates.get(filter)[2];
                ...
                if (KjcOptions.mencoder && E2Backend.steadyExecutionCounts.get(node) == 1) {
    ...
    private static void printCopyTapeElement(CodegenPrintWriter p, Tape from, Tape to) {
        if (to instanceof E2TapeDynrate) {
            p.print(to.getPushName() + "(");
            p.print("reinterpret_cast<" + ((E2TapeDynrate)(to)).getTypeCastName() + ">(");
...
//        }
        ...
        if (joiner.getType().equals(SIRJoinType.ROUND_ROBIN)
            ...
            if (KjcOptions.compressed && in.size() == 2) {
                ...
                if (E2Backend.joinerWork.containsKey(joiner)) {
                    SIRFilter filter = E2Backend.joinerWork.get(joiner);
                    JMethodDeclaration work = filter.getWork();
                    workCount = 4 / E2Backend.originalRates.get(filter)[0];
