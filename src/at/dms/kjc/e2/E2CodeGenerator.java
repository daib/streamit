...
class E2CodeGenerator {
    ...
    public E2CodeGenerator(SIROperator oper, 
        ...
        for (Tape out : data_out) {
          if (out != null && out instanceof TapeCluster) {
            if (! FixedBufferTape.isFixedBuffer(out.getSource(),out.getDest())) {
                ...
                p.println("    // this-part:"+E2Fusion.getPartition(node)+" dst-part:"+E2Fusion.getPartition(NodeEnumerator.getFlatNode(out.getDest()))+"");
            }
          }
        }   
	...
	if (! KjcOptions.standalone) {
        ...
        if (oper instanceof SIRJoiner && E2Code.feedbackJoinersNeedingPrep.contains(oper)) {
