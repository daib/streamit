 ...
 * number assigned.
 */
public class FlatIRToCluster extends InsertTimers implements StreamVisitor,
		CodeGenerator {
	/**
	...
	private int directArrayAccess = 0;
	...
	/**
	 * Code generation for a SIRFilter FlatNode. <br/>
	...
	public static void generateDirectArrayCode(SIRFilter filter) {
		FlatIRToCluster toC = new FlatIRToCluster(filter, 2);
		...
		if (E2Backend.debugging)
			System.out.println("Optimizing " + filter.getName() + "...");

		(new FinalUnitOptimize() {
			protected boolean optimizeThisMethod(SIRCodeUnit unit,
					JMethodDeclaration method) {
				return !(unit instanceof SIRTwoStageFilter && method == ((SIRTwoStageFilter) unit)
						.getInitWork());
			}
		}).optimize(filter);
	...
	public static void generateArrayCode(SIRFilter filter) {
		FlatIRToCluster toC = new FlatIRToCluster(filter, 1);
	...
	/**
	 * Code generation for a SIRFilter. <br/>
	 * For splitter and joiner see {@link ClusterCode}
	 * 
	 * @param node
	 */
	...
	private FlatIRToCluster(SIRFilter f, int directArrayAccess) {
		super();
		this.filter = f;
	...
	public void visitFilter(SIRFilter self, SIRFilterIter iter) {
		...
		// E2CodeGenerator.generateRunFunction
		...
		// selfID = NodeEnumerator.getSIROperatorId(self); // needed by the
		// class
		// } RMR

		try {
			p = new CodegenPrintWriter(new FileWriter("thread" + selfID
					+ ".cpp"));
		} catch (IOException e) {
			e.printStackTrace();
			System.exit(1);
		...
		Integer init_int = E2Backend.initExecutionCounts.get(node);
		int init_counts;
		...
		int steady_counts = E2Backend.steadyExecutionCounts.get(node)
				.intValue();
		...
		p.print("// peek: " + self.getPeekString() + " pop: "
				+ self.getPopString() + " push " + self.getPushString() + "\n");
		...
		E2CodeEstimate est = E2CodeEstimate.estimate(self);

		// int code = E2CodeEstimate.estimateCode(self);
		// int locals = E2CodeEstimate.estimateLocals(self);
		...
		E2CodeGenerator gen = new E2CodeGenerator(self, self.getFields());
		...
		if (SIRPortal.getPortalsWithReceiver(filter).length > 0) {
			...
			p.print("void handle_message__" + selfID
					+ "(netsocket *sock, int* credit);\n");
		}
		if (sendsCredits) {
		...
		p.print("void " + E2Utils.getWorkName(self, selfID) + "(int);\n\n");
		...
		if ((self instanceof SIRFileReader) || (self instanceof SIRFileWriter)) {

			if (self instanceof SIRFileReader) {
				p.println("#include <FileReader.h>");
			}
			if (self instanceof SIRFileWriter) {
				p.println("#include <FileWriter.h>");
			}

			p.println("long int __file_descr__" + selfID + ";\n");
		}
		...
		if ((self instanceof SIRFileReader) || (self instanceof SIRFileWriter)) {
			...
			if (self instanceof SIRFileReader) {
				...
				p.println("void save_file_pointer__" + selfID
						+ "(object_write_buffer *buf)");
				p.println("{ buf->write_int(FileReader_getpos(__file_descr__"
						+ selfID + ")); }\n");

				p.println("void load_file_pointer__" + selfID
						+ "(object_write_buffer *buf)");
				p.println("{ FileReader_setpos(__file_descr__" + selfID
						+ ", buf->read_int()); }\n");
			}
			...
			if (self instanceof SIRFileWriter) {
				...
				p.println("  buf->write_int(FileWriter_getpos(__file_descr__"
						+ selfID + ")); }\n");

				p.println("void load_file_pointer__" + selfID
						+ "(object_write_buffer *buf)");
				p.println("{ FileWriter_setpos(__file_descr__" + selfID
						+ ", buf->read_int()); }\n");
			}

		} else if (!(KjcOptions.mencoder || KjcOptions.blender)) {
		...
		JExpressionListStatement list = new JExpressionListStatement(null, arr,
				null);
		...
		self instanceof SIRFileReader || self instanceof SIRFileWriter) {
			...
			block.addStatement(new JForStatement(
					null,
					init,
					cond,
					(outgoing.length + incoming.length == 0 ? decr : list),
					work.getBody(),
					new JavaStyleComment[] { new JavaStyleComment(
							"FlatIRToCluster work function", true, false, false) }));
			...
			JMethodDeclaration work_n = new JMethodDeclaration(null,
					...
					work.getName(), params, CClassType.EMPTY,
					(KjcOptions.mencoder && steady_counts == 1) ? work
							.getBody() : block, null, null);
		...
		if (SIRPortal.getPortalsWithReceiver(filter).length > 0) {
			...
			if (restrictedExecution) {
				...
				while (i.hasNext()) {
					...
					p.print("__credit_" + src + "_" + selfID + " <= __counter_"
							+ selfID);
					if (i.hasNext())
						p.print(" || ");
				}
			...
			p.print("   //fprintf(stderr, \"Receiving message in " + self
					+ " on iter %d\\n\", __counter_" + selfID + ");\n");
			...
			if (portals.length > 0) {
				...
				for (int i = 0; i < pmethods.length; i++) {
					...
					CType portal_method_params[] = portal_method
							.getParameters();
		...
		if (sendsCredits) {
			...
			while (constrIter.hasNext()) {
				LatencyConstraint constraint = constrIter.next();
				...
				p.print("int __dest_offset_" + selfID + "_" + receiver
						+ " = 0;\n");
			...
			// LatencyConstraints.isRestricted(self);
			...
			while (constrIter.hasNext()) {
				...
				if (LatencyConstraints.isMessageDirectionDownstream(self,
					...
					p.print("      __msg_sock_" + selfID + "_" + receiver
							+ "out->write_int(-1);\n");
					p.print("      __msg_sock_" + selfID + "_" + receiver
							+ "out->write_int(tmp + __dest_offset_" + selfID
							+ "_" + receiver + ");\n");
					p.print("    }\n");
					p.print("    __current_" + selfID + "_" + receiver
							+ " = (__current_" + selfID + "_" + receiver
							+ " + 1) % __source_phase_" + selfID + "_"
							+ receiver + ";\n");
					p.print("    if (__current_" + selfID + "_" + receiver
							+ " == 0) __dest_offset_" + selfID + "_" + receiver
							+ " += __dest_phase_" + selfID + "_" + receiver
							+ ";\n");
					p.print("  }\n");
				} else {
					...
					p.print("    __msg_sock_" + selfID + "_" + receiver
							+ "out->write_int(-1);\n");
					p.print("    __msg_sock_" + selfID + "_" + receiver
							+ "out->write_int(__init_" + selfID + "_"
							+ receiver + ");\n");
					p.print("  } else {\n");
					p.print("    tmp = __dependency_" + selfID + "_" + receiver
							+ "[__current_" + selfID + "_" + receiver + "];\n");
		...
		Vector run = gen.generateRunFunction(filter.getInit().getName() + "__"
				+ selfID, E2Utils.getWorkName(filter, selfID), cleanupCode);
	...
	private void msgGet(CType typ, String varName) {
		if (typ instanceof CBooleanType || typ instanceof CNumericType) {
			...
			p.print("        " + varName + " = msg->get_"
				...
				&& (((CArrayType) typ).getBaseType() instanceof CBooleanType || ((CArrayType) typ)
						.getBaseType() instanceof CNumericType)) {
			// read arrays of primitive types by passing array pointer and
			// length
			...
			p.print("        msg->get_"
					...
					+ "_array_param(("
					+ CommonUtils.CTypeToString(baseType, hasBoolType) + "*)"
					+ varName + ", " + arrType.getTotalNumElements() + ");\n");
		} else {
	...
	public void visitMethodDeclaration(JMethodDeclaration self, int modifiers,
		...
		declsAreLocal = true;
		if (filter != null
				&& (filter instanceof SIRFileReader || filter instanceof SIRFileWriter)
	...
	/*
	 * I don't see why we need a new instance -- A.D. why not p.print("[");
	 * dims[i].accept(this); p.print("]");
	 */
	...
	public void visitVariableDefinition(JVariableDefinition self,
		...
		if (expr instanceof JArrayInitializer) {
			...
			// For arrays of vector type was creating code that could not be
			// converted to C.
			// For arrays from fusion was causing unnecessary overhead.
			// Assume now that compiler writers know whether variable needs
			// initailization!
				...
				if (type.isOrdinal()) {
					...
					if (((CClassType) type).toString().equals(
							"java.lang.String")) {
						p.print(" = NULL;");
					} else {
						p.print(" = {0}");
	...
	public void visitMethodCallExpression(JMethodCallExpression self,
		...
		// math functions are converted to use their floating-point
		// counterparts;
	...
	public void visitAssignmentExpression(JAssignmentExpression self,
		...
		if (right instanceof SIRPopExpression) {
			...
			if (directArrayAccess > 0) {
				Tape in = RegisterStreams.getFilterInStream(filter);
				p.print(leftString);
				p.print(" = BUFFER_");
				p.print(in.getSource());
				p.print("_");
				p.print(in.getDest());
				if (directArrayAccess == 1) {
					p.print("[");
					p.print(popCount++);
					p.print("]");
				} else {
				...
				p.print(RegisterStreams.getFilterInStream(filter)
						.assignPopToVar(leftString));
			}
			return;
		}
		...
		if (right instanceof SIRPeekExpression) {
			...
			// note following line does not work with mod_push_pop
			// but mod_push_pop does not work with arrays over tapes.
			p.print(RegisterStreams.getFilterInStream(filter).assignPeekToVar(
					leftString, argstring));
			return;
		}

		// copy arrays element-wise
		boolean arrayType = ((left.getType() != null && left.getType()
				.isArrayType()) || (right.getType() != null && right.getType()
				.isArrayType()));
		if (arrayType && !(right instanceof JNewArrayExpression)) {
			...
			try {
				...
				System.err
						.println("Warning: copy from non-array type to array type: "
								+ left + " = " + right);
			...
			assert dims != null;
			/*
			 * // if we cannot find the dim, just create a pointer copy if (dims
			 * == null) { lastLeft = left; printLParen(); left.accept(this);
			 * p.print(" = "); right.accept(this); printRParen(); return; }
			 */
	 ...
	 * visitLatency, visitLatencyMax, visitLatencyRange, visitLatencySet in ToC
	 */
	...
	public void visitMessageStatement(SIRMessageStatement self,
		...
		/*
		 * p.print("//send_" + iname + "_" + ident + "("); portal.accept(this);
		 * p.print(", "); latency.accept(this); if (params != null) for (int i =
		 * 0; i < params.length; i++) if (params[i] != null) { p.print(", ");
		 * params[i].accept(this); } p.print(");");
		 */
		...
		if (portal instanceof SIRPortal) {
			...
			for (int i = 0; i < receivers.length; i++) {
				 ...
				 * JMethodDeclaration m[] = dest_arr[t].getMethods(); for (int z
				 * = 0; z < m.length; z++) { String name = m[z].getName();
				 * boolean guess = name.startsWith(ident2);
				 * System.out.println("== "+name+" == "+(guess?"***":"")); if
				 * (guess) {
				 * System.out.println("ModState: "+ModState.methodModsState
				 * (m[z])); } } }
				 * 
				 * int index = -1;
				 */
				...
				/*
				 * if (params != null) { // int and float have size of 4 bytes
				...
				if (KjcOptions.standalone) {
					...
					p.print("  message* __msg = new message(" + size + ", "
							+ index + ", ");
					...
					if (latency instanceof SIRLatencyMax) {
						...
						if (LatencyConstraints.isMessageDirectionDownstream(
								sender, receiver)) {
							p.print("sdep_" + selfID + "_" + dst
									+ "->getDstPhase4SrcPhase(__counter_"
									+ selfID + "+" + max + "+1)");
							...
							p.print("sdep_" + selfID + "_" + dst
									+ "->getSrcPhase4DstPhase(__counter_"
									+ selfID + "+" + max + "+1)");
					...
					p.print("  __msg_stack_" + dst
							+ " = __msg->push_on_stack(__msg_stack_" + dst
							+ ");\n");
					...
					if (latency instanceof SIRLatencyMax) {
						...
						if (LatencyConstraints.isMessageDirectionDownstream(
								sender, receiver)) {
							p.print("  __msg_sock_" + selfID + "_" + dst
									+ "out->write_int(sdep_" + selfID + "_"
									+ dst + "->getDstPhase4SrcPhase(__counter_"
									+ selfID + "+" + max + "+1)");
							// subtract 1 in the downstream direction
							// because message is delivered BEFORE given iter
							p.print("-1);\n");
							...
							p.print("  __msg_sock_" + selfID + "_" + dst
									+ "out->write_int(sdep_" + selfID + "_"
									+ dst + "->getSrcPhase4DstPhase(__counter_"
									+ selfID + "+" + max + "+1)");
							// do not subtract 1 in the upstream direction
							// because message is delivered AFTER given iter
							p.print(");\n");
					...
					assert params.length == method_params.length : "Message send parameter list length "
							+ params.length
							+ " != formals list length "
							+ method_params.length;
					;
	...
	private void pushArray(SIRPushExpression self, CType tapeType,
		...
		if (outputTape instanceof E2TapeDynrate) {
			p.print(outputTape.getPushName());
			p.print("(reinterpret_cast<"
					+ ((E2TapeDynrate) (outputTape)).getTypeCastName() + ">(&");
			val.accept(this);
			p.print("))");
		} else {
			p.print(outputTape.getPushName());
