...
public class GenerateMasterDotCpp {
    ...
    public static boolean isEliminated(int i) {
        ...
        return E2Fusion.isEliminated(f)
    ...
    public static void generateMasterDotCpp() {
        ...
        p.print("static void run_join() {\n");

        for (int i = 0; i < threadNumber - 1; i++) {	//last created thread runs the join
            if (isEliminated(i)) {continue;}
            //p.print("  if (myip == init_instance::get_thread_ip("+i+")) {\n");
            p.print("    pthread_join(__pthread_"+i+", NULL);\n");
			//at thi moment, we do not need this
        ...
        */
        ...
        for (int i = 0; i < threadNumber - 1; i++) {
			...
			//read the thread ip to assign thread id
            p.print("    //pthread_attr_assign_core_id(&__pthread_attr_" + i 
					+ ", init_instance::get_thread_ip(" + i + "));\n");
