...
public class EmitCode {
    ...
    private Set<JFieldDeclaration> declaredFields = new HashSet<JFieldDeclaration>();
    ...
    public void emitCodeForComputeStore(SIRCodeUnit fieldsAndMethods,
        ...
        (new at.dms.kjc.sir.lowering.FinalUnitOptimize())
                .optimize(fieldsAndMethods);
        ...
        for (Channel c : upstreamEnds) {
            ...
            p.println("/* " + upstreamDescription(c) + " -> "
                    + downstreamDescription(c) + " */");
        ...
        for (JFieldDeclaration field : fieldsAndMethods.getFields()) {
            //            System.out.println(field.getVariable().getIdent());
            boolean found = false;
            for (JFieldDeclaration f : declaredFields) {
                if (f.getVariable().getIdent()
                        .equals(field.getVariable().getIdent())) {
                    found = true;
                    break;
                }
            }
    ...
    public void generateMain(CodegenPrintWriter p) {
        ...
        p.indent();
        p.println(backendbits.getComputeNodes().getNthComputeNode(0)
                .getComputeCode().getMainFunction().getName()
                + "();");
        p.outdent();
        p.println("}");
    }
    ...
    protected class CodeGen extends ToC implements SLIRVisitor, CodeGenerator {
        ...
        public void visitAssignmentExpression(JAssignmentExpression self,
            ...
            if ((left.getType() != null && left.getType().isArrayType())
                    || (right.getType() != null && right.getType()
                            .isArrayType())) {
        ...
        private void arrayCopy(JExpression left, JExpression right) {
            ...
            assert (var instanceof JFieldAccessExpression || var instanceof JLocalVariableExpression) : "Assigning an array to an unsupported expression of type "
                    + left.getClass() + ": " + left;
            ...
            int dims[];
            try {
                dims = CommonUtils.makeArrayInts(((CArrayType) var.getType())
                        .getDims());
            } catch (AssertionError e) {
                // Constant prop didn't get to lhs (can happen in field array = local array).
                // get dimensions from rhs.
                dims = CommonUtils.makeArrayInts(((CArrayType) right.getType())
                        .getDims());
            ...
            for (int i = 0; i < rbound; i++) {
                p.print("for (" + EmitCode.ARRAY_COPY + i + " = 0; "
                        + EmitCode.ARRAY_COPY + i + " < " + dims[i + diff]
                        + "; " + EmitCode.ARRAY_COPY + i + "++)\n");
            }
            left.accept(this);
            for (int i = 0; i < rbound; i++)
                p.print("[" + EmitCode.ARRAY_COPY + i + "]");
        ...
        public void visitMethodDeclaration(JMethodDeclaration self,
            ...
            if (!this.isDeclOnly()) {
                p.newLine();
            } // some extra space if not just declaration.
            p.newLine();
