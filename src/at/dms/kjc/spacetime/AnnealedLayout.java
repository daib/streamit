 ...
 *
 */
public class AnnealedLayout extends SimulatedAnnealing implements
        Layout<RawTile> {
    ...
    public AnnealedLayout(SpaceTimeSchedule spaceTime) {
        ...
        if (SpaceTimeBackend.NO_SWPIPELINE) {
            ...
            scheduleOrder = DataFlowOrder.getTraversal(spaceTime
                    .getPartitioner().getSliceGraph());
        } else {
    ...
    public void run() {
        ...
        for (int i = 0; i < filterList.size(); i++)
            CommonUtils.println_debugging(filterList.get(i)
                    + " is assigned to " + assignment.get(filterList.get(i)));
    ...
    public void printLayoutStats() {
        ...
        assert isLegalLayout() : "Simulated annealing arrived at an illegal layout.";
    }
    ...
    private void swapAssignmentCommPhase() {
        ...
        while (true) {
            attempts++;
            slice = partitioner.getSliceGraph()[getRandom(partitioner
                    .getSliceGraph().length)];
    ...
    public void swapAssignmentLegal() {
        ...
        assert newSliceAssignment(reassignMe.getHead().getNextFilter()) : "Error: could not find a legal layout for "
                + reassignMe + " during layout";
    ...
    private boolean newSliceAssignment(SliceNode node) {
        ...
        if (node.isOutputSlice()) {
            ...
            if (fileWriters.contains(node.getPrevious())
                    && !legalFileWriterTile(node.getPrevious().getAsFilter(),
        ...
        if (node.getPrevious().isInputSlice()) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                RawTile tile = rawChip.getTile(prevTile,
                        dirStrings[dirVec].charAt(i));
     ...
     */
    private boolean legalFileReaderTile(FilterSliceNode filter, ComputeNode tile) {
     ...
     * mapped to tile.
     */
    private boolean legalFileWriterTile(FilterSliceNode filter, ComputeNode tile) {
    ...
    private void legalInitialPlacement() {
        ...
        for (int i = 0; i < slices.length; i++) {
            ...
            newSliceAssignment(slices[i].getHead().getNextFilter());
        }
    }
    ...
    private boolean isLegalLayout() {
        ...
        while (filters.hasNext()) {
            ...
            RawTile upTile = (RawTile) assignment.get(filter.getNext()
                    .getAsFilter());
    ...
    private HashMap<RawComputeNode, Integer> distanceLatency(
        ...
        RawTile srcTile = (RawTile) assignment.get(edge.getSrc()
                .getPrevFilter());
        RawTile dstTile = (RawTile) assignment.get(edge.getDest()
                .getNextFilter());
        int multipler = 1;
        HashMap<RawComputeNode, Integer> cost = new HashMap<RawComputeNode, Integer>();
        ...
        IODevice srcPort = LogicalDramTileMapping.getHomeDram(srcTile);
        //get the por that the dest is reading from
        IODevice dstPort = LogicalDramTileMapping.getHomeDram(dstTile);
        ...
        if (bigWorkers.contains(srcTile))
            cost.put(srcTile,
                    (rawChip.manhattanDistance(srcNeighbor, dstNeighbor) + 1)
                            * multipler);
        ...
        for (int i = 0; i < slices.length; i++) {
            ...
            while (edges.hasNext()) {
                ...
                if (!IntraSliceBuffer.unnecessary(output)) {
                    StreamingDram outputDRAM = IntraSliceBuffer.getBuffer(
                            output.getPrevFilter(), output).getDRAM();

                    Iterator<RawComputeNode> route = router.getRoute(
                            outputDRAM, bufDRAM).iterator();
                    while (route.hasNext()) {
                        RawComputeNode hop = route.next();
                        if (hop instanceof RawTile)
                            commCosts.put(
                                    (RawTile) hop,
                                    commCosts.get((ComputeNode) hop)
                                            + edge.steadyItems());
                    }
                }

                if (!IntraSliceBuffer.unnecessary(input)) {
                    StreamingDram inputDRAM = IntraSliceBuffer.getBuffer(input,
                            input.getNextFilter()).getDRAM();
                    Iterator<RawComputeNode> route = router.getRoute(bufDRAM,
                            inputDRAM).iterator();
                    while (route.hasNext()) {
                        RawComputeNode hop = route.next();
                        if (hop instanceof RawTile)
    ...
    private void addHashMaps(HashMap<RawComputeNode, Integer> map1,
        ...
        while (it.hasNext()) {
            ...
            if (map1.containsKey(key)) {
                map1.put(key, map1.get(key).intValue()
                        + map2.get(key).intValue());
            }
        }
    }
    ...
    */
    ...
    private int[] getTileWorks(boolean bias) {
        ScheduleModel model = new ScheduleModel(spaceTime, this, scheduleOrder);
    ...
    public void initialize() {
        ...
        Iterator<SliceNode> sliceNodes = at.dms.kjc.slicegraph.Util
                .sliceNodeTraversal(partitioner.getSliceGraph());
        while (sliceNodes.hasNext()) {
            ...
            if (node.isFilterSlice() && !(node.getAsFilter().isPredefined())) {
                totalWork += partitioner.getFilterWorkSteadyMult(node
                        .getAsFilter());
                filterList.add(node);
            }
        }
        ...
        for (int i = 0; i < partitioner.io.length; i++) {
            ...
            if (slice.getHead().isFileOutput() && slice.getHead().oneInput()) {
                fileWriters.add(slice.getHead().getSingleEdge().getSrc()
                        .getPrevFilter());
            } else if (slice.getTail().isFileInput()
                    && slice.getTail().oneOutput()) {
                fileReaders.add(slice.getTail().getSingleEdge().getDest()
                        .getNextFilter());
            }
        }
    ...
    public static double standardDeviation(int[] vals) {
        ...
        double stdDev = Math.sqrt((squaredSum / (double) vals.length)
                - (mean * mean));
    ...
    public boolean otherSplitFiltersMapped(FilterSliceNode node,
        ...
        while (inEdges.hasNext()) {
            ...
            Iterator<InterSliceEdge> upstreamOutEdges = output.getDestSet()
                    .iterator();
            while (upstreamOutEdges.hasNext()) {
                InterSliceEdge upstreamOutEdge = upstreamOutEdges.next();
                if (assignment.containsKey(upstreamOutEdge.getDest()
                        .getNextFilter())
                        && assignment.get(upstreamOutEdge.getDest()
                                .getNextFilter()) == tile)
                    return true;
