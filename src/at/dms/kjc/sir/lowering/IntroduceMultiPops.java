  ...
  public Object visitBlockStatement(JBlock oldBlock,
	...
	if ((o instanceof JExpressionStatement)
	  ...
	  if (o instanceof JForStatement) {
	    ...
	    // RMR { sometimes singular statements are wrapped by blocks, 
	    // so if the body is a JBlock of exactly one statement, then grab the
	    // statement which should be a JExpressStatement if this loop is eligible
	    // for mpop replacement
	    if (body instanceof JBlock) {
	      if (((JBlock)body).getStatements().size() == 1) {
