...
public class KjcOptions extends at.dms.util.Options {
  ...
  public boolean processOption(int code, Getopt g) {
    ...
    case -54:
      cx = getInt(g, 0); return true;
    case -55:
      cy = getInt(g, 0); return true;
    case -56:
  ...
  public String[] getOptions() {
    ...
    String[]   total = new String[parent.length + 120];
    ...
    total[parent.length + 56] = "  --cx:                 Composed in x direction [-1]";
    total[parent.length + 57] = "  --cy:                 Composed in y direction [-1]";
    total[parent.length + 58] = "  --vectorize:          Naive vectorization for a machine with <n>-byte vector registers. [-1]";
    total[parent.length + 59] = "  --cell_vector_library: Vectorize math operations using IBMs vector versions [false]";
    total[parent.length + 60] = "  --cacheopt:           Perform cache optimization [false]";
    total[parent.length + 61] = "  --l1i:                Level 1 i-cache size in K for cache optimizations [8]";
    total[parent.length + 62] = "  --l1d:                Level 1 d-cache size in K for cache optmizations [8]";
    total[parent.length + 63] = "  --l2:                 Level 2 cache size in K for cache optimizations [256]";
    total[parent.length + 64] = "  --peekratio:          Increases multiplicity of peeking filters to make sure that (pop * ratio) >= (peek - pop) [-1]";
    total[parent.length + 65] = "  --nomult:             Do not increase multiplicity of partitions [false]";
    total[parent.length + 66] = "  --dynamicRatesEverywhere: For testing performance of dynamic rates [false]";
    total[parent.length + 67] = "  --raw, -r<int>:       Compile for RAW with a square layout, with <n> tiles per side [-1]";
    total[parent.length + 68] = "  --rawcol, -c<int>:    Sets the number of column of tiles. With this set raw specifies the number of rows. [-1]";
    total[parent.length + 69] = "  --streamit:           Compile StreamIt code. [false]";
    total[parent.length + 70] = "  --numbers, -N<int>:   Attempt Generate Raw code that gathers performance numbers, with <n> steady state cycles. [-1]";
    total[parent.length + 71] = "  --standalone, -S:     Use the Raw backend to generate C file for uniprocessor. [false]";
    total[parent.length + 72] = "  --fixseed:            Fix all Random seeds to known values for repeatable testing [false]";
    total[parent.length + 73] = "  --magic_net, -M:      Generate Magic Network Code. [false]";
    total[parent.length + 74] = "  --sjtopipe:           Convert splitjoins to pipelines. [false]";
    total[parent.length + 75] = "  --simulatework, -s:   Uses simulator to measure work required by filters. [false]";
    total[parent.length + 76] = "  --clone_with_serialization: Uses the OLD cloning method that relies on serialization. [false]";
    total[parent.length + 77] = "  --graph:              Outputs graph representation for eclipse plugin [false]";
    total[parent.length + 78] = "  --wbs:                Uses the work based simulator for laying out the communication instructions. [false]";
    total[parent.length + 79] = "  --spacetime:          Uses the spacetime partitioner. [false]";
    total[parent.length + 80] = "  --circular:           Running circular checking algorithm. [false]";
    total[parent.length + 81] = "  --cell:               Compile for Cell with <n> SPUs. [-1]";
    total[parent.length + 82] = "  --celldyn:            Compile to Cell for use with dynamic scheduler. [false]";
    total[parent.length + 83] = "  --fission:            Perform vertical fission wherever possible, with maximum of <n>-way fiss per filter. [-1]";
    total[parent.length + 84] = "  --destroyfieldarray:  Destroy field arrays [false]";
    total[parent.length + 85] = "  --outputs:            Run the application until it produces <n> outputs (RAW only). [-1]";
    total[parent.length + 86] = "  --iterations, -i<int>: Run the application for <n> steady-state iterations.. [-1]";
    total[parent.length + 87] = "  --struct:             Handle Structures. Experimental feature. [false]";
    total[parent.length + 88] = "  --backend:            Class name of the StreamIt compiler backend to run.";
    total[parent.length + 89] = "  --steadymult:         For the SpaceTime RAW backend, multiply the steady-state multiplicities by i [1]";
    total[parent.length + 90] = "  --rstream, -R:        Generate code for the RStream front-end. [false]";
    total[parent.length + 91] = "  --doloops:            Generate doloops for for loops where possible in the rstream backend. [false]";
    total[parent.length + 92] = "  --absarray:           Use abstract array syntax and semantics instead of C arrays. [false]";
    total[parent.length + 93] = "  --modfusion:          Use circular buffer with mod operations when fusing pipelines. [false]";
    total[parent.length + 94] = "  --spacedynamic:       Use the space multiplexing Raw backend with dynamic rate support. [false]";
    total[parent.length + 95] = "  --space:              Use the space Raw backend (without dynamic rate support). [false]";
    total[parent.length + 96] = "  --malloczeros:        Use mallocs instead of callocs to initial memory (malloc will zero memory). [false]";
    total[parent.length + 97] = "  --macros:             Convert small functions to macros, effectively inlining them. [false]";
    total[parent.length + 98] = "  --nopartition:        For the space-dynamic backend, do not attempt to partition each static subgraph. [false]";
    total[parent.length + 99] = "  --layoutfile:         For the space-dynamic backend, specify a layout file (a new-line separated list of tiles).";
    total[parent.length + 100] = "  --devassignfile:      For the space-dynamic backend, specify a file that assigns devices to ports (a new-line separated list of tiles).";
    total[parent.length + 101] = "  --backendvalues:      For debugging, specify class with package:value for static variable";
    total[parent.length + 102] = "  --slicethresh:        For the spacetime backend, set the slice threshold to be <int> percent. [101]";
    total[parent.length + 103] = "  --asciifileio:        Use ascii format for file input and output. [false]";
    total[parent.length + 104] = "  --ssoutputs:          When using --numbers on a dynamic-rate app, specify outputs per steady state. [-1]";
    total[parent.length + 105] = "  --noswpipe:           For spacetime, do not software pipeline the steady state. [false]";
    total[parent.length + 106] = "  --st_cyc_per_wd:      For spacetime, define the bandwidth in cycles per word for the streaming memories attached to the raw chip. [1]";
    total[parent.length + 107] = "  --hwic:               When targeting the raw simulator, enable H/W icaching. [false]";
    total[parent.length + 108] = "  --dup:                For the SpaceTime Backend, duplicate all stateless filters n times. [-1]";
    total[parent.length + 109] = "  --noswitchcomp:       For the SpaceTime Backend, turn off switch instruction compression. [false]";
    total[parent.length + 110] = "  --autoparams:         For the SpaceTime Backend, automatically calculate parameters. [false]";
    total[parent.length + 111] = "  --greedysched:        For the SpaceTime Backend, use a greedy bin packing algorithm to calculate the schedule. [false]";
    total[parent.length + 112] = "  --workestunroll:      For the SpaceTime Backend, perform unrolling on filters for work estimation. [false]";
    total[parent.length + 113] = "  --rawdcachesize:      For Raw, set the dcache size for each tile (use a power of 2). [32768]";
    total[parent.length + 114] = "  --rawicachesize:      For Raw, set the dcache size for each tile (use a power of 2). [32768]";
    total[parent.length + 115] = "  --localstoglobals:    Convert local variables to global variables (can avoid stack overflow). [false]";
    total[parent.length + 116] = "  --checkpointSIR:      Generate STR code from SIR. [false]";
    total[parent.length + 117] = "  --nompops:            Do not convert existing loops that simply pop() to pop(n) statements. [false]";
    total[parent.length + 118] = "  --lime:               Generate code for Lime compiler. [false]";
    total[parent.length + 119] = "  --simulator:          Compile for simulator [false]";
  ...
  private static final LongOpt[] LONGOPTS = {
    ...
    new LongOpt("cx", LongOpt.REQUIRED_ARGUMENT, null, -54),
    new LongOpt("cy", LongOpt.REQUIRED_ARGUMENT, null, -55),
    new LongOpt("vectorize", LongOpt.REQUIRED_ARGUMENT, null, -56),
    new LongOpt("cell_vector_library", LongOpt.NO_ARGUMENT, null, -57),
    new LongOpt("cacheopt", LongOpt.NO_ARGUMENT, null, -58),
    new LongOpt("l1i", LongOpt.REQUIRED_ARGUMENT, null, -59),
    new LongOpt("l1d", LongOpt.REQUIRED_ARGUMENT, null, -60),
    new LongOpt("l2", LongOpt.REQUIRED_ARGUMENT, null, -61),
    new LongOpt("peekratio", LongOpt.REQUIRED_ARGUMENT, null, -62),
    new LongOpt("nomult", LongOpt.NO_ARGUMENT, null, -63),
    new LongOpt("dynamicRatesEverywhere", LongOpt.NO_ARGUMENT, null, -64),
    new LongOpt("raw", LongOpt.REQUIRED_ARGUMENT, null, 'r'),
    new LongOpt("rawcol", LongOpt.REQUIRED_ARGUMENT, null, 'c'),
    new LongOpt("streamit", LongOpt.NO_ARGUMENT, null, -65),
    new LongOpt("numbers", LongOpt.REQUIRED_ARGUMENT, null, 'N'),
    new LongOpt("standalone", LongOpt.NO_ARGUMENT, null, 'S'),
    new LongOpt("fixseed", LongOpt.NO_ARGUMENT, null, -66),
    new LongOpt("magic_net", LongOpt.NO_ARGUMENT, null, 'M'),
    new LongOpt("sjtopipe", LongOpt.NO_ARGUMENT, null, -67),
    new LongOpt("simulatework", LongOpt.NO_ARGUMENT, null, 's'),
    new LongOpt("clone_with_serialization", LongOpt.NO_ARGUMENT, null, -68),
    new LongOpt("graph", LongOpt.NO_ARGUMENT, null, -69),
    new LongOpt("wbs", LongOpt.NO_ARGUMENT, null, -70),
    new LongOpt("spacetime", LongOpt.NO_ARGUMENT, null, -71),
    new LongOpt("circular", LongOpt.NO_ARGUMENT, null, -72),
    new LongOpt("cell", LongOpt.REQUIRED_ARGUMENT, null, -73),
    new LongOpt("celldyn", LongOpt.NO_ARGUMENT, null, -74),
    new LongOpt("fission", LongOpt.REQUIRED_ARGUMENT, null, -75),
    new LongOpt("destroyfieldarray", LongOpt.NO_ARGUMENT, null, -76),
    new LongOpt("outputs", LongOpt.REQUIRED_ARGUMENT, null, -77),
    new LongOpt("iterations", LongOpt.REQUIRED_ARGUMENT, null, 'i'),
    new LongOpt("struct", LongOpt.NO_ARGUMENT, null, -78),
    new LongOpt("backend", LongOpt.REQUIRED_ARGUMENT, null, -79),
    new LongOpt("steadymult", LongOpt.REQUIRED_ARGUMENT, null, -80),
    new LongOpt("rstream", LongOpt.NO_ARGUMENT, null, 'R'),
    new LongOpt("doloops", LongOpt.NO_ARGUMENT, null, -81),
    new LongOpt("absarray", LongOpt.NO_ARGUMENT, null, -82),
    new LongOpt("modfusion", LongOpt.NO_ARGUMENT, null, -83),
    new LongOpt("spacedynamic", LongOpt.NO_ARGUMENT, null, -84),
    new LongOpt("space", LongOpt.NO_ARGUMENT, null, -85),
    new LongOpt("malloczeros", LongOpt.NO_ARGUMENT, null, -86),
    new LongOpt("macros", LongOpt.NO_ARGUMENT, null, -87),
    new LongOpt("nopartition", LongOpt.NO_ARGUMENT, null, -88),
    new LongOpt("layoutfile", LongOpt.REQUIRED_ARGUMENT, null, -89),
    new LongOpt("devassignfile", LongOpt.REQUIRED_ARGUMENT, null, -90),
    new LongOpt("backendvalues", LongOpt.REQUIRED_ARGUMENT, null, -91),
    new LongOpt("slicethresh", LongOpt.REQUIRED_ARGUMENT, null, -92),
    new LongOpt("asciifileio", LongOpt.NO_ARGUMENT, null, -93),
    new LongOpt("ssoutputs", LongOpt.REQUIRED_ARGUMENT, null, -94),
    new LongOpt("noswpipe", LongOpt.NO_ARGUMENT, null, -95),
    new LongOpt("st_cyc_per_wd", LongOpt.REQUIRED_ARGUMENT, null, -96),
    new LongOpt("hwic", LongOpt.NO_ARGUMENT, null, -97),
    new LongOpt("dup", LongOpt.REQUIRED_ARGUMENT, null, -98),
    new LongOpt("noswitchcomp", LongOpt.NO_ARGUMENT, null, -99),
    new LongOpt("autoparams", LongOpt.NO_ARGUMENT, null, -100),
    new LongOpt("greedysched", LongOpt.NO_ARGUMENT, null, -101),
    new LongOpt("workestunroll", LongOpt.NO_ARGUMENT, null, -102),
    new LongOpt("rawdcachesize", LongOpt.REQUIRED_ARGUMENT, null, -103),
    new LongOpt("rawicachesize", LongOpt.REQUIRED_ARGUMENT, null, -104),
    new LongOpt("localstoglobals", LongOpt.NO_ARGUMENT, null, -105),
    new LongOpt("checkpointSIR", LongOpt.NO_ARGUMENT, null, -106),
    new LongOpt("nompops", LongOpt.NO_ARGUMENT, null, -107),
    new LongOpt("lime", LongOpt.NO_ARGUMENT, null, -108),
    new LongOpt("simulator", LongOpt.NO_ARGUMENT, null, -109)
  };
}
