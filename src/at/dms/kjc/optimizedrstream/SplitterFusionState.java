...
public class SplitterFusionState extends FusionState {
    ...
    private void calculateRemaining() {
        ...
        itemsSentFrom = StrToRStream.getMult(node, true) * distinctRoundItems();

        remaining[0] = itemsSentTo - itemsSentFrom;

        if (remaining[0] > 0)
            System.out
                    .println("** Items remaining on an incoming splitter buffer");
    ...
    private boolean setNecessary() {
        ...
        if (node.isDuplicateSplitter() && node.ways > 0) {
            ...
            int bufferSizeSteady = FusionState.getFusionState(
                    node.getEdges()[0]).getBufferSize(node, false);
            int bufferSizeInit = FusionState.getFusionState(node.getEdges()[0])
                    .getBufferSize(node, true);
            ...
            if (FusionState.getFusionState(node.getEdges()[0]).getRemaining(
                    node, true) > 0) {
            ...
            for (int i = 1; i < node.ways; i++) {
                ...
                if (FusionState.getFusionState(node.getEdges()[0])
                        .getRemaining(node, true) > 0) {
                    //System.out.println("Downstream peek buffer > 0");
                    return true;
                }
                //if all buffer size's are equal, good
                if (bufferSizeSteady != FusionState.getFusionState(
                        node.getEdges()[i]).getBufferSize(node, false)) {
                    //System.out.println("Unequal buffer sizes of children (steady)");
                    return true;
                }
                if (bufferSizeInit != FusionState.getFusionState(
                        node.getEdges()[i]).getBufferSize(node, true)) {
    ...
    public void initTasks(Vector<JFieldDeclaration> fields,
            Vector<JMethodDeclaration> functions, JBlock initFunctionCalls,
            JBlock main) {
        ...
        if (!necessary) {
            for (int i = 0; i < node.ways; i++)
                ((FilterFusionState) FusionState
                        .getFusionState(node.getEdges()[i]))
    ...
    **/
    public JStatement[] getWork(JBlock enclosingBlock, boolean isInit) {
        ...
        JavaStyleComment[] comment = { new JavaStyleComment(
                splitter.toString(), true, false, false) };
        ...
        if (node.isDuplicateSplitter()) {
            statements.addStatement(getDuplicateCode(enclosingBlock, mult,
                    isInit));
        } else {
        ...
        if (remaining[0] > 0) {
            ...
            JVariableDefinition loopCounterBackup = GenerateCCode.newIntLocal(
                    BACKUPCOUNTER, myUniqueID, 0);
            ...
            statements.addStatement(remainingBackupLoop(bufferVar[0],
                    loopCounterBackup, StrToRStream.getMult(node, isInit)
                            * distinctRoundItems(), remaining[0]));
        }
    ...
    private JStatement getRRCode(JBlock enclosingBlock, int mult, boolean isInit) {
        ...
        JVariableDefinition induction = GenerateCCode.newIntLocal(RRCOUNTER,
                myUniqueID, 0);
        ...
        for (int i = 0; i < node.ways; i++) {
            ...
            JVariableDefinition innerVar = GenerateCCode.newIntLocal(RRINNERVAR
                    + myUniqueID + "_", i, 0);
            //get the downstream fusion state
            FusionState downstream = FusionState
                    .getFusionState(node.getEdges()[i]);
            ...
            if (blending) {
                ...
                JAddExpression outgoingIndex = new JAddExpression(null,
                        new JMultExpression(null, new JLocalVariableExpression(
                                null, induction), new JIntLiteral(
                                node.weights[i])),
                        new JLocalVariableExpression(null, innerVar));
    ...
    **/
    private JStatement getDuplicateCode(JBlock enclosingBlock, int mult,
            boolean isInit) {
        ...
        JVariableDefinition induction = GenerateCCode.newIntLocal(
                DUPLICATECOUNTER, myUniqueID, 0);
        ...
        for (int i = 0; i < node.ways; i++) {
            ...
            JArrayAccessExpression rhs = new JArrayAccessExpression(null,
                    incomingBuffer, new JLocalVariableExpression(null,
                            induction));

            JExpression outgoingIndex = new JLocalVariableExpression(null,
                    induction);
            ...
            JArrayAccessExpression lhs = new JArrayAccessExpression(null,
                    outgoingBuffer, outgoingIndex);

            JExpressionStatement assignment = new JExpressionStatement(null,
                    new JAssignmentExpression(null, lhs, rhs), null);
            assigns.addStatement(assignment);
        }

        return GenerateCCode.makeDoLoop(assigns, induction, new JIntLiteral(
                mult));
    }
    ...
    private JVariableDefinition makePopBuffer() {
        ...
        return makeBuffer(itemsAccessed, CommonUtils.getOutputType(node),
