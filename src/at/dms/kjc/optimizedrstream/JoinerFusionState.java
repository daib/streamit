...
public class JoinerFusionState extends FusionState {
    ...
    private void calculateRemaining() {
        ...
        for (int i = 0; i < node.inputs; i++) {
            ...
            itemsReceived = (StrToRStream.getMult(node.incoming[i], true) * Util
                    .getItemsPushed(node.incoming[i], node));
            ...
            if (remaining[i] > 0)
                System.out
                        .println("** Items remaining on an incoming joiner buffer "
                                + i);

            assert remaining[i] >= 0 : "Error: negative value for items remaining after init on channel";
        }
    ...
    public void initTasks(Vector<JFieldDeclaration> fields,
        ...
        if (isFBJoiner) {
            ...
            JVariableDefinition index = GenerateCCode.newIntLocal(
                    INIT_PATH_INDEX, myUniqueID, 0);
            ...
            JExpression rhs = new JMethodCallExpression(null,
                    new JThisExpression(null), initPath.getName(), args);
            //the buffer access
            JExpression lhs = new JArrayAccessExpression(null,
                    new JLocalVariableExpression(null, loopBackBuf),
                    new JLocalVariableExpression(null, index));

            JAssignmentExpression assign = new JAssignmentExpression(null, lhs,
                    rhs);
            //place the assignments in a loop
            initFunctionCalls.addStatement(GenerateCCode.makeDoLoop(
                    new JExpressionStatement(null, assign, null), index,
    ...
    public JStatement[] getWork(JBlock enclosingBlock, boolean isInit) {
        ...
        for (int i = 0; i < node.inputs; i++) {
            ...
            JVariableDefinition innerVar = GenerateCCode.newIntLocal(
                    JOINERINNERVAR + myUniqueID + "_", i, +0);
            //add the decl of the induction variable
            if (!KjcOptions.doloops)
                enclosingBlock
                        .addStatementFirst(new JVariableDeclarationStatement(
        ...
        for (int i = 0; i < node.inputs; i++) {
            ...
            if (getRemaining(node.incoming[i], isInit) > 0 && offset > 0) {
                ...
                statements.addStatement(remainingBackupLoop(
                        getBufferVar(node.incoming[i], isInit), induction,
                        offset, getRemaining(node.incoming[i], isInit)));
                //number of items
    ...
    public int getRemaining(FlatNode prev, boolean isInit) {
        ...
        if (!isInit && node.isFeedbackIncomingEdge(node.getIncomingWay(prev))) {
            return remaining[node.getIncomingWay(prev)]
                    - (StrToRStream.getMult(node, isInit) * node
                            .getIncomingWeight(prev));
        }
