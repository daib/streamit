...
public class FFSNoPeekBuffer extends FilterFusionState {
    ...
    private void setNecessary() {
        ...
        if (filter instanceof SIRIdentity && remaining[0] == 0) {
    ...
    private void createVariables() {
        ...
        popCounterVar = new JVariableDefinition(null, 0, CStdType.Integer,
                POPCOUNTERNAME + myUniqueID, new JIntLiteral(-1));
        ...
        if (filter.getPushInt() > 0) {
            ...
            pushCounterVar = new JVariableDefinition(null, 0, CStdType.Integer,
                    PUSHCOUNTERNAME + myUniqueID, new JIntLiteral(
                            next.getRemaining(node, false) - 1));

            pushCounterVarInit = new JVariableDefinition(null, 0,
                    CStdType.Integer, PUSHCOUNTERNAME + myUniqueID,
                    new JIntLiteral(-1));
    ...
    public JStatement[] getIndexDecls(boolean isInit) {
        ...
        if (filter.getPopInt() > 0)
            stmts.add(new JVariableDeclarationStatement(null, popCounterVar,
                    null));
        if (filter.getPushInt() > 0)
            stmts.add(new JVariableDeclarationStatement(null,
                    isInit ? pushCounterVarInit : pushCounterVar, null));
    ...
    private int getLastProducedInit() {
        ...
        return Util.getItemsPushed(last, node)
                * StrToRStream.getMult(last, true);
    ...
    private JStatement getPopBufDecl() {
        ...
        return new JVariableDeclarationStatement(null, bufferVar[0], null);
    }
    ...
    private JVariableDefinition makePopBuffer() {
        ...
        int mult = StrToRStream.getMult(node, false) > StrToRStream.getMult(
                node, true) ? StrToRStream.getMult(node, false) : StrToRStream
                .getMult(node, true);
        ...
        return makeBuffer(bufferSize, filter.getInputType(), BUFFERNAME
                + myUniqueID);
    }
    ...
    public JStatement[] getWork(JBlock enclosingBlock, boolean isInit) {
        ...
        if (StrToRStream.getMult(getNode(), isInit) > 0) {
            ...
            JVariableDefinition forIndex = GenerateCCode.newIntLocal(
                    FORINDEXNAME, myUniqueID, 0);
            ...
            body = GenerateCCode.makeDoLoop(body, forIndex, new JIntLiteral(
                    mult));

            //see if can generate MIV buffer indices, if true, then the conversion 
            //was performed
            ConvertChannelExprsMIV tryMIV = new ConvertChannelExprsMIV(this,
                    isInit);
            if (!StrToRStream.GENERATE_MIVS
                    || !tryMIV.tryMIV((JDoLoopStatement) body)) {
                if (StrToRStream.GENERATE_MIVS && KjcOptions.doloops) {
                    ...
                    System.out.println("Could not generate MIV indices for "
                            + getNode().contents);
                }
        ...
        if (remaining[0] > 0 && StrToRStream.getMult(node, isInit) > 0) {
            ...
            statements.addStatement(remainingBackupLoop(bufferVar[0],
                    loopCounterBackup,
                    filter.getPopInt() * StrToRStream.getMult(node, isInit),
                    remaining[0]));
        }
